...
- Boom Game module (isolated):
  ...
  - Middle medals: in each game the three players in the middle of the posting order each get a bonus entry in the draw. With an even player count the centre two always get one and the third goes at random to the post just before or after them; with three or fewer players everyone gets one. A medallist is listed twice, keeps their better draw, and the `n` players are then scored `n`…1 as usual. Medallists get a :sports_medal: reaction and a :sports_medal: marker in the results. Posting first or last earns nothing extra
