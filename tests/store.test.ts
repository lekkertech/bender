...
describe('Store random point assignment', () => {
  ...
  it('labels the middle three to post, and keeps the labels on the stored awards', () =>
    inRandomEra((db) => {
      const d = DAY;
      const offsets: Record<string, number> = { U1: 0, U2: 60, U3: 240, U4: 300, U5: 540 };
      ...
      expect(medals).toEqual({ U1: undefined, U2: 'middle', U3: 'middle', U4: 'middle', U5: undefined });
      expect(awards.map((a) => a.points)).toEqual([5, 4, 3, 2, 1]);
  ...
  it('scores one entrant a single point, and settles an unplayed game empty', () =>
    inRandomEra((db) => {
      ...
      expect(db.resolveGame(d, 'boom').map((a) => [a.points, a.medal])).toEqual([[1, 'middle']]);
