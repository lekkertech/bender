...
| `src/features/boom/random/index.ts` | Randomised points: a fixed 12:00-12:10 window, every unique entrant draws a distinct value in 1..n when it settles; the three in the middle of the posting order draw twice and keep the better draw. |
| `src/features/boom/random/announce.ts` | The randomised path's results post, its draw audit thread reply, and Friday crown. |
| `src/features/boom/random/audit.ts` | Renders the draw audit: per game, each player's post position and time, medal, tickets drawn and points. |
