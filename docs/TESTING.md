...
- Boom module (`BOOM_SCORING=random`, the default):
  ...
  - Timing medals: in each game the first to post, the last to post, and the entrant nearest the halfway point between them get a `:sports_medal:` reaction and a `:sports_medal:` marker after their points in the results post. With one entrant only that message is medalled; with two, both; with three or more, exactly three. Verify the marked players match `awards[date][game][].medal` in `data/store.json`. Their scores are still within 1..`n`.
  - Draw audit: a thread reply appears under the daily results post. For each game, every player appears once with their post time, their medal if any, one ticket (two for a medallist, highest first), and points. Ranks follow the kept ticket from highest to lowest, and the tickets match `awards[date][game][].draws` in `data/store.json`.
