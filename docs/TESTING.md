...
- Boom module (`BOOM_SCORING=random`, the default):
  ...
  - When the window closes, each of the `n` unique entrants gets a unique random amount between 1 and `n` (verify no duplicate amounts and no gaps), and the top three earners get `:first_place_medal:` / `:second_place_medal:` / `:third_place_medal:` reactions.
  - Middle medals: in each game the three players in the middle of the posting order get a `:sports_medal:` reaction and a `:sports_medal:` marker after their points in the results post. With an odd count that is the exact centre three; with an even count the centre two plus one neighbour. With three or fewer players everyone is medalled. The first and last to post are never medalled once five or more play. Verify the marked players match `awards[date][game][].medal` in `data/store.json`. Their scores are still within 1..`n`.
  - Draw audit: a thread reply appears under the daily results post. For each game, every player appears once with their post position and time, their medal if any, one ticket (two for a medallist, highest first), and points. Ranks follow the kept ticket from highest to lowest, and the tickets match `awards[date][game][].draws` in `data/store.json`.
