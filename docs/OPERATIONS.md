...
- Timers and recovery:
  - Windows are closed by an in-process timer. If the bot restarts mid-window, the window is
    settled by a background sweep (every 30s) or by the next message in the channel, using the
    channel recorded with the entries.
- Migration:
  - On first start, the Store stamps `random_scoring_from` with the current local date. Dates
    before it keep legacy 3-2-1 podium scoring, so historical leaderboards and crowns are
    unchanged; dates from it on are scored only from settled `awards`.
